...
<head>
    ...
    <title>Report: Customer Genre Popularity</title>
</head>
<body>
<h1>Customer Genre Popularity</h1>
<p>This report returns all the genre names in the database ordered based on popularity by the avg rating given by all the customers in the target birth year (age)</p>
<form action="" method="post">
    <p>
        <label for="target_birth_year">Select Birth Year (age): </label>
...
if(isset($_POST["target_birth_year"])) {
    h2("Result");
    $tbl_cols = array('genrename', 'avg rating');
    $target_birth_year = getBirthYearAge($_POST["target_birth_year"]);
    span("Target Birth Year (age): ".$target_birth_year);
    $sql = "select ".
                "g.genrename ".
                ",round(avg(gr.rating),1) as avg_genre_rating ".
            "from ".
                "f21_s001_20_genre_rating  gr ".
                "inner join f21_s001_20_genre g ".
                    "on gr.genreid = g.genreid ".
            "where ".
                "gr.custid in (select distinct c.custid from f21_s001_20_customer c where extract(year from c.dob) = :p1) ".
            "group by ".
                "g.genrename ".
            "order by ".
